...
from embedding_sequencers.embedding_sequencer import run_embedding_sequencer_multi_seed

# Default seeds to use for multi-seed runs
DEFAULT_SEEDS = [0, 1, 2, 3, 4]


class EmbeddingSequencersSeedsRunner(threadward.Threadward):
    """
    Threadward runner for embedding sequencers with multiple random seeds.
    
    Runs 5 seeds per configuration and saves aggregated results (mean, std, var)
    to seed_results.json in the same folder structure as the original runner.
    """
    
    def __init__(self, debug=False, results_folder="../embedding_sequencers_results", seeds=None):
    ...
    def task_method(self, variables, task_folder, log_file):
        ...
        # Call the multi-seed embedding sequencer
        run_embedding_sequencer_multi_seed(
    ...
    def verify_task_success(self, variables, task_folder, log_file):
        # Check for seed_results.json instead of results.json
        return os.path.exists(os.path.join(task_folder, "seed_results.json"))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Run embedding sequencers with multiple seeds via threadward'
    )
    ...
    parser.add_argument('--seeds', type=int, nargs='+', default=None,
                       help='List of seeds to use (default: 0 1 2 3 4)')
    parser.add_argument('--num-seeds', type=int, default=5,
                       help='Number of seeds if --seeds not provided (uses 0 to num_seeds-1)')
...
if __name__ == "__main__":
    ...
    runner = EmbeddingSequencersSeedsRunner(
        debug=args.debug, 
